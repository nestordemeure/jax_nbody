...
# -------------------------------
# Acceleration Computation using xmap
# -------------------------------
...
def compute_accelerations(pos, mass, G, softening):
    ...
    # while leaving the spatial axis ("d") unchanged.
    force_matrix = xmap(
        body_force,
        in_axes={
            "pos_i": ["i", ...],
    ...
    # force_matrix has shape (N, N, d): force_matrix[i, j, :] is the force on body i due to body j.
    
    # Use xmap to subtract out the self-interaction for each body.
    # For each body "i", subtract the diagonal element from the sum over j.
    net_acc = xmap(
    ...
    # The above xmap is applied for each body "i". The resulting net_acc has shape (N, d).
    return net_acc
...
plt.xlabel("x")
plt.ylabel("y")
plt.title(f"Final Positions of Bodies (energy:{energy:.2f} time:{runtime:.3f}s)")
